...
const exibeBotaoAtualizar = (nomeUsuario, emailUsuario, dadosUsuario) => {
  ...
  dadosAlteradosPermissao = Array.from(
    permissaoUsuario.getElementsByTagName("input")
  ).some((radio) => {
    if (radio.checked) {
  ...
  if (exibirBotao) botaoUpdate.classList.remove("hidden");
  else botaoUpdate.classList.add("hidden");
...
botaCadastrar.addEventListener("click", async function (event) {
  ...
  try {
    ...
    footer.innerHTML = `${await api.cadastrarUsuario(
      cadastroNome.value,
      cadastroEmail.value,
      cadastroCpf.value,
      cadastroSenha.value
...
botaoLogin.addEventListener("click", async () => {
  ...
  try {
    ...
    if (resposta?.pagina) {
      ...
      const selectEleicao = criarSelectComLabel(
        "lista-eleicoes",
      ...
      containerDadosMenu.classList.add(
        "w-full",
        "flex-1",
        "flex",
        "flex-col",
      ...
      if (permissao <= 1) {
        ...
        menuNav = criarMenuNav(
          ["Dados", "Eleitores", "Candidatos", "Eleição", "Resultado"],
          opcoes,
          dadosEntrada,
          containerDadosMenu
        );
