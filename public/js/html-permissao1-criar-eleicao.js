...
function htmlPermissao1CriarEleicao(tituloPadrao) {
    ...
    const { label: titulo, input: tituloConteudo } = criarElemento("Título", "Digite o título da eleição", "titulo-eleicao", "text");
    const { label: descricao, input: descricaoConteudo } = criarElemento("Descrição", "Descreva a eleição", "descricao-eleicao", "text");
    const { label: cnpj, input: cnpjConteudo } = criarElemento("CNPJ", "CNPJ da eleição", "cnpj-eleicao", "text");
    const { label: dataInicio, input: dataInicioConteudo } = criarElemento("Data início", "Data e hora de início", "dataInicio-eleicao", "datetime-local");
    const { label: dataFim, input: dataFimConteudo } = criarElemento("Data fim", "Data e hora de fim", "dataFim-eleicao", "datetime-local");
    ...
    botaoCriarEleicao.addEventListener("click", async () => {
        if (!validarCNPJ(cnpjConteudo.value)) {
            alert("CNPJ inválido! Digite um CNPJ válido no formato: XX.XXX.XXX/XXXX-XX");
            return;
        }
    ...
    [titulo, tituloConteudo, descricao, descricaoConteudo, cnpj, cnpjConteudo, dataInicio, dataInicioConteudo, dataFim, dataFimConteudo, botaoCriarEleicao]
