...
function criarDefinicaoEleitores(tipo, dadosEntrada, dadosSaida) {
    ...
    section.classList.add("relative", "w-full", "h-screen", "bg-indigo-200", "flex", "flex-col", "items-center", "p-4");

    const titulo = document.createElement("p");
    titulo.classList.add("bg-white", "text-lg", "font-bold", "text-center", "p-4", "w-full");
    ...
    containerLista.classList.add("relative", "w-full", "flex-grow", "flex", "justify-center", "items-center", "pb-20"); // Ajusta padding inferior para o botão

    function criarLista(id, itens) {
        const div = document.createElement("div");
        div.classList.add("w-80", "h-[calc(100vh-250px)]", "bg-indigo-100", "p-2", "border", "overflow-y-auto"); // Ajusta a altura
        div.id = id;
    ...
    botoes.classList.add("absolute", "top-1/2", "left-1/2", "-translate-x-1/2", "-translate-y-1/2", "flex", "flex-col", "gap-2", "z-10", "bg-indigo-200", "p-2", "rounded-lg", "shadow-lg");

    function criarBotao(id, imgPath, moverPara, tudo) {
        const botao = document.createElement("div");
        botao.id = id;
        botao.classList.add("w-10", "h-10", "bg-no-repeat", "bg-cover", "bg-center", "bg-indigo-200", "hover:bg-indigo-100", "cursor-pointer", "rounded-md");
        ...
        botao.addEventListener("click", () => {
            ...
            let selecionados = tudo ? Array.from(listaOrigem.querySelectorAll("ul")) : Array.from(listaOrigem.querySelectorAll("ul.bg-indigo-400"));
    ...
    const botaoTudoDireita = criarBotao("botao-tudo-direita", "/assets/seta-dupla-direita.png", "saida", true);
    const botaoTudoEsquerda = criarBotao("botao-tudo-esquerda", "/assets/seta-dupla-esquerda.png", "entrada", true);
    ...
    botaoConfirmar.classList.add(
        "absolute", "bottom-4", "left-1/2", "-translate-x-1/2", "w-3/4", "bg-indigo-600", "text-white", "p-3", "text-center", "hover:bg-indigo-700", "transition", "rounded-md"
    );
